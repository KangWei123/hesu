(function() {
  let domUtils = UM.dom.domUtils;
  let widgetName = 'video';
  ...
  UM.registerWidget(widgetName, {
    ...
    updateAlignButton: function(align) {
      let aligns = $('#eduiVideoFloat', this.$widget)[0].children;
    ...
    createAlignButton: function(ids) {
      ...
      for (var i = 0, ci; (ci = ids[i++]); ) {
        let floatContainer = $('#' + ci, this.$widget)[0],
    ...
    switchSelect: function(selectParentId) {
      let selects = $('#' + selectParentId, this.$widget)[0].children;
    ...
    findFocus: function(id, returnProperty) {
      let tabs = $('#' + id, this.$widget)[0].children,
        property;
    ...
    endWith: function(str, endStrArr) {
      for (let i = 0, len = endStrArr.length; i < len; i++) {
        let tmp = endStrArr[i];
