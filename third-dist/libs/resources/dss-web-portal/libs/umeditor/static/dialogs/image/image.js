(function() {
  ...
  var Base = {
    ...
    scale: function(img, max, oWidth, oHeight) {
      ...
      let height = 0;
      let percent;
      let ow = img.width || oWidth;
      let oh = img.height || oHeight;
      if (ow > max || oh > max) {
        if (ow >= oh) {
          if ((width = ow - max)) {
        ...
        .on('click', function() {
          if (
            $(this)
              .parent()
              .remove()
              .hasClass('edui-image-upload-item')
          ) {
            // 显示图片计数-1
            Upload.showCount--;
            Upload.updateView();
    ...
    callback: function(editor, $w, url, state) {
      if (state == 'SUCCESS') {
        // 显示图片计数+1
        Upload.showCount++;
        let $img = $("<img src='" + editor.options.imagePath + url + "' class='edui-image-pic' />");
        let $item = $(
  ...
  var Upload = {
    ...
    uploadComplete: function(r) {
      ...
      try {
        ...
        let lang = me.editor.getLang('image');
      ...
      $(me.dialog).delegate('.edui-image-file', 'change', function(e) {
        ...
        if (me.editor.getOpt('useCustomUpload')) {
          ...
          $('<iframe name="up"  style="display: none"></iframe>')
            ...
            .on('load', function() {
              let r = this.contentWindow.document.body.innerHTML;
              if (r == '') return;
              me.uploadComplete(r);
              $(this).unbind('load');
              $(this).remove();
    ...
    toggleMask: function(html) {
      ...
      let $mask = $('.edui-image-mask', me.dialog);
      if (html) {
        if (!(UM.browser.ie && UM.browser.version <= 9)) {
        ...
        .on('click', function() {
          ...
          if (url) {
            $("<img src='" + url + "' class='edui-image-pic' />").on('load', function() {
              let $item = $("<div class='edui-image-item'><div class='edui-image-close'></div></div>").append(this);
  ...
  UM.registerWidget(
    'image',
    {
      ...
      initContent: function(editor, $dialog) {
        let lang = editor.getLang('image').static,
          opt = $.extend({}, lang, {
      ...
      buttons: {
        ok: {
          exec: function(editor, $w) {
            ...
            let list = Base.getAllPic(sel, $w, editor);
