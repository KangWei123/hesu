...
(function() {
  ...
  var manageTextarea = (function() {
    ...
    return function manageTextarea(el, opts) {
      ...
      if (!opts) opts = {};
      let textCallback = opts.text || noop;
      let keyCallback = opts.key || noop;
  ...
  let digits = Parser.digits = regex(/^[0-9]*/);
    var whitespace = (Parser.whitespace = regex(/^\s+/));
    var optWhitespace = (Parser.optWhitespace = regex(/^\s*/));

  let any = Parser.any = Parser(function(stream, onSuccess, onFailure) {
  ...
  var Fragment = P(function(_) {
    ...
    let rightEnd = self.ends[R]
      let parent = leftEnd.parent;
  ...
  var MathElement = P(Node, function(_, _super) {
    ...
    _.bubble = function(event /*, args... */) {
      let args = __slice.call(arguments, 1);

      for (let ancestor = this; ancestor; ancestor = ancestor.parent) {
        let res = ancestor[event] && ancestor[event].apply(ancestor, args);
        if (res === false) break;
      }
  ...
  */
    _.numBlocks = function() {
      let matches = this.htmlTemplate.match(/&\d+/g);
    ...
    _.html = function() {
      ...
      let cmdId = ' mathquill-command-id=' + cmd.id;
      let tokens = cmd.htmlTemplate.match(/<[^<>]+>|[^<>]+/g);

      pray('no unmatched angle brackets', tokens.join('') === this.htmlTemplate);

      // add cmdId to all top-level tags
      for (let i = 0, token = tokens[0]; token; i += 1, token = tokens[i]) {
  ...
  function createRoot(jQ, root, textbox, editable) {
    let contents = jQ.contents().detach();
    ...
    //textarea stuff
  let textareaSpan = root.textarea = $('<span class="textarea"><textarea></textarea></span>'),
    ...
    let latex = cursor.selection ? '$'+cursor.selection.latex()+'$' : '';
  ...
  let anticursor, blink = cursor.blink;
  ...
  var RootTextBlock = P(MathBlock, function(_) {
    ...
    let mathMode = string('$').then(latexMathParser)
      ...
      let textChar = escapedDollar.or(regex(/^[^$]/)).map(VanillaSymbol);
      var latexText = mathMode.or(textChar).many();
    let commands = latexText.skip(eof).or(all.result(false)).parse(latex);
  ...
  for (let prop in transformPropNames) {
  ...
  if (transformPropName) {
    ...
    scale = function(jQ, x, y) {
      ...
      let intervalId = setInterval(calculateMarginRight);
  ...
  var SupSub = P(MathCommand, function(_, _super) {
    ...
    _.respace = function() {
      if (
        this[L].ctrlSeq === '\\int ' ||
        (this[L] instanceof SupSub && this[L].ctrlSeq != this.ctrlSeq && this[L][L] && this[L][L].ctrlSeq === '\\int ')
      ) {
        if (!this.limit) {
          this.limit = true;
      ...
      if (this.respaced) {
      let fontSize = +this.jQ.css('fontSize').slice(0,-2),
...
let Vec = LatexCmds.vec = P(MathCommand, function(_, _super) {
    _.ctrlSeq = '\\vec';
    _.htmlTemplate =
      '<span class="non-leaf">' +
...
CharCmds['|'] = P(Paren, function(_, _super) {
  ...
  var InnerTextBlock = P(MathBlock, function(_, _super) {
    ...
    _.blur = function() {
      this.jQ.removeClass('hasCursor');
      if (this.isEmpty()) {
      let textblock = this.parent, cursor = textblock.cursor;
...
CharCmds['\\'] = P(MathCommand, function(_, _super) {
    ...
    let latex = this.ends[L].latex(), cmd;
...
LatexCmds.vector = P(MathCommand, function(_, _super) {
    ...
    _.onKey = function(key, e) {
      let currentBlock = this.cursor.parent;
  ...
  var LatexFragment = P(MathCommand, function(_) {
    ...
    _.parser = function() {
      let frag = latexMathParser.parse(this.latex).children();
...
*/
  ...
  (function() {
    let trig = ['sin', 'cos', 'tan', 'sec', 'cosec', 'csc', 'cotan', 'cot'];
    for (let i in trig) {
      LatexCmds[trig[i]] = LatexCmds[trig[i] + 'h'] = LatexCmds['a' + trig[i]] = LatexCmds['arc' + trig[i]] = LatexCmds[
        'a' + trig[i] + 'h'
  ...
  var latexMathParser = (function() {
    ...
    function joinBlocks(blocks) {
      let firstBlock = blocks[0] || MathBlock();
    ...
    let symbol = regex(/^[^${}\\_^]/).map(VanillaSymbol);
  ...
  let mathGroup = string('{').then(function() { return mathSequence; }).skip(string('}'));
  ...
  var Cursor = P(Point, function(_) {
    ...
    function moveUpDown(self, dir) {
      if (self[R][dir]) self.insAtLeftEnd(self[R][dir]);
        ...
        let ancestorBlock = self.parent;
        do {
          let prop = ancestorBlock[dir];
          if (prop) {
            ...
            if (prop === false || prop instanceof MathBlock) {
              ...
              if (prop instanceof MathBlock) {
                let cached = self.upDownCache[prop.id];
    ...
    _.seek = function(target, pageX, pageY) {
      clearUpDownCache(this);
    let cmd, block, cursor = this.clearSelection().show();
    ...
    function offset(self) {
      ...
      let offset = self.jQ.removeClass('cursor').offset();
    ...
    let oneA = this, otherA = anticursor; //one ancestor, the other ancestor
      ...
      if (left[R] !== right) {
        for (let rightward = left; rightward; rightward = rightward[R]) {
  ...
  jQuery.fn.mathquill = function(cmd, latex) {
    switch (cmd) {
      ...
      case 'cmd':
        if (arguments.length > 1)
          return this.each(function() {
            let blockId = $(this).attr(mqBlockId),
              block = blockId && MathElement[blockId],
              cursor = block && block.cursor;
