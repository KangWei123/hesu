<template>
  <div class="enter-rate-line"
       v-loading="loading">
    <card-title text="入店率"
                tip-name="enterRate">
      <template slot="right">
        <i :title="enterViewType === 'chart' ? '表格视图' : '趋势视图'"
    ...
    <div class="customer-chart"
         v-if="enterViewType === 'chart'">
      <div class="line-content">
        <!--fixme week和day结构暂时是一样的-->
        <template v-if="flag === 'day' || flag === 'week'">
          <place-holder v-if="!enterChartData"
                        :height="380" />
          <chart-empty :options="enterChartData"
                       v-else
                       height="100%" />
        </template>
    ...
    <template v-else-if="enterViewType === 'table'">
      <enter-rate-table :data="enterTableData"
                        v-if="enterTableData" />
    </template>
  </div>
</template>
...
export default {
  ...
  methods: {
    getEnterRate() {
      this.loading = true;
      const dateRange = this.dateSection;

      storeApi
        .getEnterRate(dateRange.startDate, dateRange.endDate, this.value, this.deviceType)
        .then(result => {
