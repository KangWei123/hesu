<template>
  <div class="flow-panel">
    ...
    <div class="border" v-bind:class="{ 'border-left': borderLeft, 'border-right': borderRight }"></div>
  </div>
</template>
...
export default {
  ...
  watch: {
    'data.passengerFlowCount'(val, oVal) {
      ...
      let arr = this.formatNum(val).split('');
      this.moveY = [];
...
.flow-panel {
  ...
  .border-left {
    border-left: 5px dotted @border-blue;
  }

  .border-right {
    border-right: 5px dotted @border-blue;
  }
}
</style>
