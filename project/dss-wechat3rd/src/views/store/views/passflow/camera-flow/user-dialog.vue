  ...
  >
    <div class="user-from">
      <div class="head">
        ...
        <div class="user-box">
          <div class="user-item">
            <span>用户ID：</span>{{ userData.id }} <span style="margin-left: 20px">姓名：</span>{{ userData.name }}
          </div>
          <div class="user-item">
            <span>性别：</span>{{ userData.sex }} <span style="margin-left: 60px">年龄：</span>{{ userData.age }}
          </div>
          <div class="user-item"><span>手机：</span>{{ userData.phone || '无' }}</div>
          <div class="user-item">
            <span>是否会员：</span>{{ userData.memberType ? '是' : '否' }}
            <a class="dss-link" @click="gotoMember" href="javascript:;" v-if="userData.memberType && userData.memberId"
              >查看更多会员信息</a
            >
          </div>
        </div>
      ...
      <div class="user-foot">
        ...
        <div class="user-item">
          <span>到店次数：{{ totalCount || userData.enterCount }}</span>
        </div>
        ...
        <el-pagination
          ...
          :total="totalCount"
          v-if="totalCount > pageParams.pageSize"
...
<script>
  ...
  import { memberApi } from '@/register/member';
  export default {
    name: 'userDialog',
    components: {},
    props: {
    ...
    methods: {
      ...
      gotoMember() {
        window.location.href = '/member.html#/members/list/detail?id=' + this.userData.memberId;
      },
      ...
      //获取到店记录
      getRecord() {
        let params = { ...this.pageParams, customerId: this.userData.id };
        memberApi
          .getStoreLog(params)
          .then(result => {
