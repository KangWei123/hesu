<template>
  <div class="mp-is-attached" v-loading="loading" element-loading-spinner="el-icon-loading">
    <div class="op-code">
      <span class="txt">如果小程序运行不正常，可以尝试重新授权</span>
      <span class="btn" :title="this.allowOperate ? '' : '暂无权限操作'" @click="reAuth">重新授权</span>
    </div>

    <div class="op-code">
      <span class="txt">如果需要将小程序绑定到其它店铺，请先解绑。</span>
      <span class="btn" :title="this.allowOperate ? '' : '暂无权限操作'" @click="unBind">解除绑定</span>
    </div>

    <div class="op-block" v-if="!online.version">
      <div class="icon icon-red"></div>
      <div class="title">开发版本</div>

      <div class="status" :class="dev.status in ableFailDesc ? 'fail-status' : ''">
        {{ !dev.version ? '暂无开发版' : StatusName[dev.status] }}
      </div>
      <div class="version" v-if="dev.version">版本号：{{ dev.version }}</div>
      <div class="time">{{ dev.time | time }}</div>

      <el-popover v-if="dev.qrCode" placement="top" width="200" trigger="hover">
        <img style="width: 200px" :src="'data:image/jpeg;base64,' + dev.qrCode" />
        <a slot="reference" href="javascript:;" class="table-href">体验二维码</a>
      </el-popover>

      <el-popover
        v-if="dev.status in ableFailDesc"
        placement="top"
        title="失败原因"
        width="200"
        trigger="hover"
        :content="dev.desc"
      >
        <div class="reason" slot="reference">失败原因</div>
      </el-popover>
    ...
    <div class="op-block" v-if="online.version">
      ...
      <span class="version" v-if="online.version"> V {{ online.version }}</span>
      <div class="version" v-else>暂无已上线版本</div>

      <img style="width: 140px" class="online-qr-img" :src="online.qrCode" />
...
<script>
  ...
  export default {
    computed: {
      ...
      allowOperate() {
        const editButton = scopeMenuEnum.getMenuScopeByIdentifier(
          this.wkbScopeMenu,
          scopeMenuEnum.dictionary.wkbChildMenu.wkbWxSettingMenu.clientButton.client_applet_authorization_edit
        );
    ...
    methods: {
      filerStatus(status) {
        for (const devKey in Status) {
          if (
            Status[devKey] == status ||
            (typeof Status[devKey] === 'object' && Status[devKey].hasOwnProperty(status))
          ) {
            return devKey;
          }
        }
      ...
      clickReUploadMinPro() {
        if (this.allowOperate) {
          ...
          settingsApi
            .uploadMinPro({ sellerTemplateId: this.dev.id })
            .then(() => {
              this.$message({ type: 'success', message: '配置已更新' });
              this.checkStatus();
            })
      ...
      clickReleaseMinPro() {
        if (this.allowOperate) {
          this.submitting = true;
          settingsApi
            .releaseMinPro({ sellerTemplateId: this.dev.id })
            .then(() => {
      ...
      reAuth() {
        if (this.allowOperate) {
          const authUrl = 'https://www.wakecloud.com/wp/auth/authQRCode?authType=' + storeConst.WX_AUTH_TYPE.MA;
      ...
      apiUnBindApp() {
        const params = {
          authorizerAppId: this.wxAuthInfo.wxMaAppId,
        };
