...
export default {
  ...
  methods: {
      ...
      service.getUserTemplate().then(response => {
        if (response.success === true) {
          ...
          if (response.data.tabBarConfig) {
            ...
            this.wxConfig = JSON.parse(response.data.codeExtConfig);
          }
          ...
          if (!(this.customPages && this.customPages.length)) {
            ...
            console.log('changePageLinkRange', this.customPages);
    ...
    save() {
      ...
      // let wxTabbarConfig = this.wxConfig.tabBar.list;
      ...
      this.wxConfig.tabBar.list = wxTabbarConfig;
      //上传微信
      this.uploadWxConfig();
    },

    uploadWxConfig() {
      this.$confirm('导航栏修改需要再次发布小程序,确认是否保存？', '确认', {
        ...
        .then(() => {
          ...
          service.createNewDev().then(response => {
            ...
            const params = {
              ...
              wxExtJson: JSON.stringify(this.wxConfig),
            ...
            service.updateTemplate(params).then(res => {
              ...
              this.changePageLinkRange();
              // 3、上传新的配置数据到微信
              const param = { sellerTemplateId: this.curTemplateId };
              settingsAPI
                .uploadMinPro(param)
                .then(() => {
