<template>
  <div class="group-management">
    ...
    <el-tabs v-model="activeTab" class="group-list-box">
...
export default {
  ...
  data() {
    return {
      ...
      tabs: [
        ...
        { name: 'doing', label: '砍价中', status: [cutpriceStatus.ON_GOING] },
        {
          name: 'successed',
          label: '砍价成功',
          status: [cutpriceStatus.DONE, cutpriceStatus.PAID, cutpriceStatus.REFUNDING, cutpriceStatus.REFUND_COMPLETED]
        },
        {
          name: 'failed',
          label: '砍价失败',
          status: [cutpriceStatus.TIME_OUT]
        }
      ],
      // 当前激活的状态
      activeTab: 'all',
  ...
  created() {
    ...
    if (query && query.activityStatus) {
      this.activityStatus = parseInt(query.activityStatus);
    }
  },
