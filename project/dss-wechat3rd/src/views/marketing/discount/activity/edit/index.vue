  ...
  <div class="discount-edit" v-loading="isLoading">
    <el-breadcrumb separator="/" class="wkt-breadcrumb-container" style="border-bottom: 1px solid #dcdee0;">
      <template v-if="!!deleteShow">
        <el-breadcrumb-item :to="{ path: '/marketing/discount/discard' }">删除的活动</el-breadcrumb-item>
        <el-breadcrumb-item>详情</el-breadcrumb-item>
      </template>
      <template v-else>
        <el-breadcrumb-item :to="{ path: '/marketing/discount/activity/list' }">活动管理</el-breadcrumb-item>
    ...
    <el-form :model="form" :rules="rules" label-width="120px" ref="elForm" size="mini" class="discount-edit-form"     :disabled="!!isDisabled">
      ...
      <el-form-item label="优惠设置："
        ...
        <el-radio-group v-model="form.sameItemSwitch">
          <div class="radio-box">
            ...
            <div class="tip" >购买了不同的商品，价格低的享受此优惠，例：买了2件商品，第2件半价可以是不同的商品。</div>
          </div>
          <div class="radio-box">
            <el-radio :label="distcountEnum.sameItemSwitchStatus.NO">限同款</el-radio>
            <div class="tip" >购买了同款商品可享受此优惠，例：买了2件商品，第2件半价是相同的商品。</div>
          </div>
        </el-radio-group>
      </el-form-item>
     ...
     <el-form-item label="优惠内容："
        ...
        <el-radio-group v-model="form.activityContent">
          <div class="radio-box">
            <el-radio :label="distcountEnum.activityContentEnum.halfFare.value">第2件半价</el-radio>
            <div class="tip " >多件商品都参与活动时，价格低的商品享受半价优惠，优惠可循环叠加，例：买了1个商品A（10元）和商品B（20元），则商品A享受半价优惠。</div>
          </div>
          <div class="radio-box">
            <el-radio :label="distcountEnum.activityContentEnum.Send.value">第二件0元</el-radio>
            <div class="tip" >多件商品都参与活动时，价格低的商品享受免单优惠，优惠可循环叠加。例：买了1个商品A（10元）和商品B（20元），则商品A享受免单优惠。</div>
          </div>
          <div class="radio-box">
            <el-radio :label="distcountEnum.activityContentEnum.custom.value">自定义优惠</el-radio>
            <div class="tip" >例：第3件8折，第2件减10元。</div>
            <div v-if="form.activityContent===distcountEnum.activityContentEnum.custom.value">
              <span>第</span>
              <el-input style="width:112px;margin:0 10px;"
                          v-model="form.ladder.count"/>
              <span>件</span>
              <el-radio-group v-model="form.ladder.discountType" style="margin:25px 0  0 54px;">
                  <div class="radio-discount-box">
                    <el-radio :label="distcountEnum.discountTypeEnum.discount">打<el-input style="width:112px;margin:0 10px;"  v-model="form.ladder.discount"/>折</el-radio>
                  </div>
                  <div class="radio-discount-box">
                    <el-radio :label="distcountEnum.discountTypeEnum.money">减<el-input style="width:112px;margin:0 10px;"  v-model="form.ladder.price"/>元</el-radio>
                  </div>
              </el-radio-group>
            </div>
          </div>
...
export default {
  ...
  data() {
    ...
    const activityValidator = (rule, value, callback) => {
      if (this.form.activityContent === distcountEnum.activityContentEnum.custom.value) {
        if (!positiveInteger.reg.test(this.form.ladder.count)) {
          return callback(positiveInteger.msg);
        }
        if (this.form.ladder.discountType === distcountEnum.discountTypeEnum.discount) {
          if (!positiveNumber.reg.test(this.form.ladder.discount)) {
            return callback(positiveNumber.msg);
          }
        } else {
    ...
    return {
      ...
      form: {
        ...
        ladder: {
          discountType: distcountEnum.discountTypeEnum.discount,
          count: null,
          discount: null,
          price: null
        }
  ...
  methods: {
    ...
    setFormData(data) {
      ...
      if (this.form.activityContent === distcountEnum.activityContentEnum.custom.value) {
        ...
        this.form.ladder.discountType = data.ladderDTOList[0].discountType;
        if (this.form.ladder.discountType === distcountEnum.discountTypeEnum.discount) {
          this.form.ladder.discount = data.ladderDTOList[0].discount / 10; // 优惠金额/折扣/固定金额
        } else {
    ...
    onSave() {
      this.$refs.elForm.validate(valid => {
        ...
        let ladderDTO = {};
        //第二件半价
        if (this.form.activityContent === distcountEnum.activityContentEnum.halfFare.value) {
          ladderDTO.discountByTotalSwitch = 0; //是否按总价金额计算 0单件价格为基础计算 1总价为基础计算
          ladderDTO.discount = 50; // 优惠金额/折扣/固定金额,折扣百分制
          ladderDTO.discountType = 0; //优惠类型 为0时 DISCOUNT是折扣 为1 时候 DISCOUNT为扣减金额  2固定金额
          ladderDTO.threshold = 2; //门槛,满多少件或者满多少钱
          ladderDTO.thresholdType = 0; //门槛类型 0几件 1 多少钱
          params.ruleName = '第二件半价';
        }
        //第二件0元，等于第二件0折
        if (this.form.activityContent === distcountEnum.activityContentEnum.Send.value) {
          ladderDTO.discountByTotalSwitch = 1; //是否按总价金额计算 0单件价格为基础计算 1总价为基础计算
          ladderDTO.discount = 0; // 优惠金额/折扣/固定金额,折扣百分制
          ladderDTO.discountType = 0; //优惠类型 为0时 DISCOUNT是折扣 为1 时候 DISCOUNT为扣减金额  2固定金额
          ladderDTO.threshold = 2; //门槛,满多少件或者满多少钱
          ladderDTO.thresholdType = 0; //门槛类型 0几件 1 多少钱,
          params.ruleName = '第二件0元';
        }
        //自定义优惠，
        if (this.form.activityContent === distcountEnum.activityContentEnum.custom.value) {
          ladderDTO.discountByTotalSwitch = 0; //是否按总价金额计算 0单件价格为基础计算 1总价为基础计算
          ladderDTO.thresholdType = 0; //门槛类型 0几件 1 多少钱
          ladderDTO.discountType = this.form.ladder.discountType; //优惠类型 为0时 DISCOUNT是折扣 为1 时候 DISCOUNT为扣减金额  2固定金额
          ladderDTO.threshold = this.form.ladder.count; //门槛,满多少件或者满多少钱
          if (this.form.ladder.discountType === distcountEnum.discountTypeEnum.discount) {
            ladderDTO.discount = this.form.ladder.discount * 10; // 优惠金额/折扣/固定金额
            params.ruleName = '第' + this.form.ladder.count + '件打' + this.form.ladder.discount + '折';
          } else {
            ladderDTO.discount = this.form.ladder.price * 100; // 优惠金额/折扣/固定金额
            params.ruleName = '第' + this.form.ladder.count + '件减' + this.form.ladder.price + '元';
          }
        }
        ladderDTOList.push(ladderDTO);
        params.ladderDTOList = ladderDTOList;
        //第二件半价
        params.activityType = distcountEnum.activityTypeEnum.SECOND_HALF.value;
        ...
        discountApi[key](params).then(res => {
          if (res.data) {
            ...
            this.$router.push('/marketing/discount/activity/list');
            return;
          }
        });
      });
