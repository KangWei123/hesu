  ...
  <div class="wkb-order-list">
    ...
    <div class="order-list-box">
      ...
      <order-list-table v-loading="loading"
                        show-patch
                        :order-type="orderTab.ORDER_TYPE.CHARGE"
  ...
  methods: {
    queryUserPower() {
      if (this.wkbScopeMenu) {
        const chargeOrderButton = scopeMenuEnum.dictionary.wkbChildMenu.wkbOrderMgrMenu.chargeOrderButton;

        const scopeImportChargeOrder = scopeMenuEnum.getMenuScopeByIdentifier(
          this.wkbScopeMenu,
          chargeOrderButton.import_charge_order
        );
        this.isPowerExport = !!scopeImportChargeOrder && !!scopeImportChargeOrder.status;
      }
    },
    // 查询订单列表
    listOrders() {
      ...
      if (!params['itemTypeList[0]']) {
        params['itemTypeList[0]'] = orderStatusEnum.ORDER_TYPE.CHARGE.value;
        params['itemTypeList[1]'] = orderStatusEnum.ORDER_TYPE.MANUAL_CHARGE.value;
      }
