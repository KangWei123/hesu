<template>
  <el-dialog
    title="产品发货"
    class="dss-dialog-wrapper dss-dialog-wrapper-extension"
    v-loading="loading"
    :visible="true"
    :append-to-body="true"
    :show-close="false"
    width="600px"
  >
    <el-form :model="shippingInfo" :rules="shippingRules" label-width="100px" ref="shippingForm">
      <el-form-item label="物流选择" prop="shippingType">
        <el-radio
          v-model="shippingInfo.shippingType"
          size="mini"
          v-if="expressType"
          :label="orderStatusEnum.SHIPPING_TYPE.THIRD_PARTY"
          >三方物流
        </el-radio>
        <el-radio v-model="shippingInfo.shippingType" size="mini" :label="orderStatusEnum.SHIPPING_TYPE.NO_NEED"
          >无需物流
        </el-radio>
      </el-form-item>
      <el-form-item
        v-if="shippingInfo.shippingType === orderStatusEnum.SHIPPING_TYPE.THIRD_PARTY"
        label="物流单号"
        prop="shippingNo"
      >
        <el-input
      ...
      >
        <el-select style="width: 194px" v-model="shippingInfo.shippingCompany" placeholder="请选择物流公司" size="mini" auto-complete="off">
...
import orderApi from '@/dss-wechat3rd/src/api/order.js';
  ...
  data() {
    ...
    return {
      ...
      shippingRules: {
        ...
        // ,message: [{ message: '商家留言不能超过50个字', trigger: 'change', min: 0, max: 50 }]
      },
  ...
  methods: {
    ...
    handleSure() {
      ...
      this.$refs.shippingForm.validate(valid => {
        if (valid) {
          this.loading = true;
          orderApi[methods](this.shippingInfo)
            .done(res => {
              this.$emit('done');
            })
            .always(() => {
