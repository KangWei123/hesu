      ...
      this.$router.__proto__.switchPage= function(path,otherPage = ''){
        console.log('switchPage',path,otherPage)
          if(config.adapterIfrNoGap){
            ...
            config.notifyIfmParent(
              'urlchange',
              otherPage ? otherPage : location.origin+location.pathname+"#"+path
            );
          }
          else {
            window.location.href=
              otherPage ? otherPage : location.origin+location.pathname+location.search+"#"+path;
          }
