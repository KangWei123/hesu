<template>
  <div class="wkt-sub-nav-container" :class="{ scroll: hasScroll, 'adapter-no-overflow': adapterIfrNoGap }">
    <!-- <div class="sub-nav"
         :class="adapterIfrNoGap?'sub-nav-pos':''"
         v-if="visibleNav&&!isHideNav">

      <div v-for="(item,index) in menuList"
           :key="index"
           @click="tabSwitch(item, index)"
           :class="{active: active(item), disabled: item.disabled}">
        <template v-if="item.disabled">
          <el-popover placement="left"
                      width="200"
                      trigger="hover"
                      :content="scopeMenuEnum.disabledTip || ''">
    ...
    >
      <div class="sub-nav-content" :class="[dark ? 'dark-nav-content' : '']">
        <div
          ...
          v-if="showThirdNav && isThirdMenu(menuList && menuList[currentIndex] && menuList[currentIndex].childMenu)"
          style=""
        >
          <div class="is-top">
            <div class="nav-wrap">
