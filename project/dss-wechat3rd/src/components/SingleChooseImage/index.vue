  ...
  >
    <template v-if="imgUrl">
      ...
      <div
        class="avatar"
        :style="{
          background: 'transparent url(' + imgUrl + ') no-repeat center center / cover',
          maxWidth: width + 'px',
          height: height + 'px',
        }"
      >
        <span class="upload-desc" @click.stop="handleChooseImage" v-if="canReset">点击重选</span>
        <el-button class="btn-img-del" icon="el-icon-delete" circle @click.stop="handleDelete"></el-button>
      </div>
    </template>

    <div :style="{ cursor: !isDisabledEdit ? 'pointer' : 'not-allowed' }" v-else @click.stop="handleChooseImage">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete() {
        this.isDeleteConfirm ? this.onRemoveConfirm() : this.onRemove();
      },
