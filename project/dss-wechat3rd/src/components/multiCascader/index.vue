    ...
    >
      <div slot="reference">
        <el-select
          ...
          popper-class="multicascader-hide-popper"
          @focus="handleFocus"
      ...
      <div class="multicascader-cascader-menu-wrapper" v-clickoutside="hidePopover">
        <div  v-if="options.length > 0">
          ...
          >
            <li
              ...
              'el-cascader-menu__item--extensible': item['childCount'] > 0,
              'can-load-children': !item.isLeaf && !item[childrenKey] && item['childCount'] > 0 && allowLoadChildren && showLoadingIndicator,
            ...
            >
              ...
              ></el-checkbox>
              <span style="margin-right: 20px">{{ item[labelKey] }}</span>
              <!-- <div style="float: right; margin-right: 15px;">
                <i class="el-icon-edit" @click.stop="editItem(item)" style="margin-right: 5px"></i>
        ...
        <ul class="el-cascader-menu cascader-menu" v-else>
          <li class="el-cascader-menu__item dropdown__empty">{{ noDataText }}</li>
        </ul>
        <!-- <div class="multicascader-el-cascader-add">
          <div class="multicascader-el-cascader-add__btn" @click.stop="addLabelType">+新增标签类型</div>
          <div class="multicascader-el-cascader-add__btn" @click.stop="addLabel" v-show="casTree.length > 1">+新增标签</div>
        </div>-->
      </div>
    </el-popover>
  </div>
...
export default {
  ...
  created() {
    ...
    this.popOverClass = `multicascader-cascader-popper ${this.classRef} ${this.popperClass}`;
    this.initOpts();
    this.initDatas();
  },
  mounted() {
  ...
  methods: {
    ...
    markParentChecked(node) {
      ...
      function loop(node) {
        ...
        if (hasArrayChild(node, vm.childrenKey)) {
          ...
          // 子节点全部被选中
          if (checkCount === node[vm.childrenKey].length && node.parent) {
            node.checked = true;
            node.indeterminate = false;
          } else {
            node.checked = false;
    ...
    async spreadNext(children, index, item) {
      //spreadNext(children, index, item) {
      // console.log('async spreadNext');
      const vm = this;
      if (
      ...
      if ((index || index === 0) && item.childCount > 0) {
        if (vm.casTree.indexOf(children) === -1) {
          ...
          vm.$emit('spread', (vm.currentItem = item));
        }
      }
    },
    addLabel() {
      this.hidePopover();
      // let data = this.casTree[1][0].parent;
      // this.$emit('addLabel', vm.currentItem);
      this.$emit('addLabel', this.currentItem);
    },
    addLabelType() {
      this.hidePopover();
      this.$emit('addLabelType');
    },
...
<style>
.multicascader-hide-popper {
  display: none;
}
.multicascader-cascader-popper {
  padding: 0px;
}

.multicascader-cascader-menu-wrapper {
  white-space: nowrap;
  overflow-x: auto;
}
...
.multicascader-cascader-menu-wrapper .el-cascader-menu {
...
.cascader-wrapper .el-popover__reference:focus {
  outline: none;
}
.dropdown__empty {
  height: 100%;
...
.can-load-children.loading-children::after {
  animation: multi-cascader-loading 0.22s infinite alternate;
  -moz-animation: multi-cascader-loading 0.22s infinite alternate; /* Firefox */
  -webkit-animation: multi-cascader-loading 0.22s infinite alternate; /* Safari 和 Chrome */
  -o-animation: multi-cascader-loading 0.22s infinite alternate; /* Opera */
}
.multicascader-el-cascader-add {
  display: flex;
...
.multicascader-el-cascader-add .multicascader-el-cascader-add__btn {
  flex: 1;
  text-align: center;
}
@keyframes multi-cascader-loading {
  from {
    background: #a5d279;
  }
  to {
...
/deep/ .el-tag--mini .el-select__tags-text{
  vertical-align: top;
  display: inline-block;
  max-width: 200px;
  overflow: hidden;
