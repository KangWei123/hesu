<template>
  ...
  <el-form inline class="normal-filter" :model="filter" ref="filter">
    <el-form-item label="关键字：" v-if="showKeyword">
      <el-input placeholder="关键字搜索" v-model="filter.itemName" />
    </el-form-item>
    ...
    <template v-for="(item, index) in filterList">
      <el-form-item
        v-if="!!item.type && goodsType.type !== 'scoreActivity'"
        :label="(item.showFormLabel && item.label) || ''"
        :key="index"
      >
        <!-- 选择器 -->
        <el-select
