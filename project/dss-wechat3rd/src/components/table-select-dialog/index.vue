...
<script>
  ...
  export default {
    ...
    mixins: [tableCheckedHelperMixin, AdapterIfr.AdapterMixins],
    model: {
      prop: 'showDialog',
    },
        ...
        if (!this.apiUrl) {
          ...
          try {
            ...
            if (this.requestMethod == 'post') {
              res = await services.json(this.apiUrl, params, {
                action: '获取列表数据',
              });
            } else {
              res = await services.arrayGet(this.apiUrl, {
                params,
                action: '获取列表数据',
              });
            }
        ...
        if (this.isClickAll) {
          ...
          try {
            if (this.requestMethod == 'post') {
              ...
              res = await services.get(this.apiUrl, {
                params,
                action: '获取列表数据',
              });
            }
