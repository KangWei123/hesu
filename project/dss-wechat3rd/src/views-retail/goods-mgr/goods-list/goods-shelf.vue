...
export default {
  ...
  data() {
    return {
      ...
      detailPath: '/retail/goods-mgr/shelf/goods-shelf/detail' // 详情页面路径
    };
  },
  methods: {
    queryUserPower() {
      if (this.wkbScopeMenu) {
        const goodsButton = scopeMenuEnum.dictionary.wkbChildMenu.wkbGoodsMgrMenu.storeShelfMenu.goodsButton;
        // 查看
        const scopeShowGoodsButton = scopeMenuEnum.getMenuScopeByIdentifier(
          this.wkbScopeMenu,
          goodsButton.store_goods_show
        );
