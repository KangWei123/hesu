<template>
  <div class="haggle-module-edit">
    <edit-header :data-source="dataSource" name="代金卡包"></edit-header>
    ...
    <div class="com-body">
      ...
      <commonEdit
        ...
        :show-label="false"
        :custom-bg-mode="true"
        :nav-page="linkedPages.page.CARD_PACK"
      ...
      <div class="img-preview-container img-preview-margin">
        <div class="img-preview" v-for="(item, index) in dataSource.data" :key="index" :draggable="true">
          <div class="img" :style="{ 'background-image': 'url(' + item.itemImageUrls[0] + ')' }"></div>
  ...
  export default {
    ...
    methods: {
      onChooseGoods(choosedGoodsList) {
        console.log('choosedGoodsList', choosedGoodsList, utils.saveAvailableFields(choosedGoodsList, fields.cardPack));
        // 删除不需要的字段
        this.dataSource.data = utils.saveAvailableFields(choosedGoodsList, fields.cardPack);
      },
