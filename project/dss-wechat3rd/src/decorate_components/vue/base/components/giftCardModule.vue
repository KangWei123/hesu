    ...
    watch: {
      'dataSource.data': {
        ...
        handler(newVal, oldVal) {
          this.nowIndex = 0;
          if (newVal) {
            this.addWkbImage(newVal, 'bgImg', 'wkBgImg');
          }
        },
      },
    },
    ...
    methods: {
      getList() {
        ...
        if (data && data.length) {
          service
            .itemNosForCardList({
              itemNo: utils.pickUpItemNos(data, 'id').join(','),
            })
            .then(res => {
              // eslint-disable-next-line vue/no-mutating-props
              this.dataSource.data = utils.saveAvailableFields(res.data, fields.giftCard);
            });
        }
      },
