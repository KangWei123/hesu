...
<script>
  ...
  import dealerDialog from '@/mod-org/src/components/dealer-dialog/index';
  import WorkHourPicker from '@/mod-org/src/components/WorkHourPicker/index.vue';
  import SelectTree from '@/mod-org/src/components/tree-select';
  ...
  import { DELIVERY_TYPES } from '@/mod-org/src/constants/delivery-way';
  import dealerEnum from '@/mod-org/src/constants/dealerEnum';
      ...
      return {
        ...
        rules: {
          ...
          tel: [
            ...
            {
              pattern:
                /(^((13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[013,5-8]|18[0-9]|19[0-9])[0-9]{8})|([0-9]{3,4}-[0-9]{7,8})$)|(^400-[016789]\d{2}-\d{4}$)|(^400[016789]\d{6}$)/,
          ...
          .then(res => {
            ...
            if (res.data)
              for (const key in this.form) {
                if (!(key in hold)) {
                  if (undefined !== res.data[key] && key !== 'area') {
