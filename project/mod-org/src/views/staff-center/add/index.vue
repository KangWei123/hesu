...
<script>
  import TreePermission from '@/mod-org/src/components/PermissionTree/permission-tree';
  ...
  import { getStoreList } from '@/mod-org/src/api/storeApi';
  ...
  export default {
      ...
      const validatePosition = (rule, value, callback) => {
        if (!value.role) {
          ...
          callback(
            new Error(
              `请选择${
                this.isDealerManager ? '经销商' : this.operator || this.isAreaManagerTag ? '组织节点' : this.storeLabel
              }`
            )
          );
        } else {
    ...
    methods: {
      ...
      findParents(arr, target) {
        ...
        const dfs = (tmpArr, tmpTarget, path) => {
          for (let i = 0; i < tmpArr.length; i++) {
            if (res) break;
            if (tmpArr[i].deptId === tmpTarget) {
              res = tmpArr[i];
              parent = path.concat(tmpTarget);
              return;
            }
            if (Array.isArray(tmpArr[i].children)) {
              dfs(tmpArr[i].children, tmpTarget, path.concat(tmpArr[i].deptId));
            }
          }
        };
        dfs(arr, target, []);
      ...
      // 获取门店列表
      async getStoreData() {
        const { data } = await getStoreList({ searchCount: true });
        this.storeList = data || [];
      },
      ...
      formatData(arr) {
        const dfs = tmpArr => {
          for (let i = 0; i < tmpArr.length; i++) {
            tmpArr[i].disabled = false;
            if (tmpArr[i].type !== 3) {
              tmpArr[i].disabled = true;
            }
            if (Array.isArray(tmpArr[i].children)) {
              dfs(tmpArr[i].children);
            }
          }
        };
        dfs(arr);
