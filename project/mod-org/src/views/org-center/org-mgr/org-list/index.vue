<template>
  <list-page-layout class="store-list-container">
    <template #main-actions>
      <el-button type="primary" v-if="isShowAddButton" :disabled="hasAddStore" @click="clickAddOrg">
        新增{{ storeLabel }}
      </el-button>
    </template>
    ...
    <template #filters>
      <el-form ref="query" :model="filterForm" inline>
        <el-form-item label="项目搜索" prop="storeName">
          <el-input
    ...
    <template #sub-actions>
      <el-button :disabled="disableBatchButton || disableBatchExport" @click="clickExportOrg">批量导出</el-button>
    </template>
    ...
    <div class="store-container" v-loading="treeloading">
      ...
      <div class="table-box">
        <div>
          {{ `${isGlobalSearch ? '筛选出' : currentNodeName} 组织 ${totalCount} 家` }}
        </div>
        <el-table
          :data="tableData"
          ref="multipleTable"
        ...
        >
          ...
          <el-table-column width="200" label="项目所属地区">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" min-width="110" fixed="right">
            <template slot-scope="scope">
              ...
              <a href="javascript:;" class="table-href link-del" @click="clickDelDetail(scope.row)">删除</a>
            </template>
          </el-table-column>
        </el-table>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      // 按钮权限-新增组织
      isShowAddButton() {
        //  scrm业态的账号创建组织时，只要组织数量大于0，则不允许继续添加组织
        if (this.isScrmIndustry && (this.totalCount > 0 || this.tableData.length > 0)) {
          return false;
        } else {
          const orgManageButton = scopeMenuEnum.dictionary.companyChildMenu.orgCenterChildMenu;
          const tempMenu = scopeMenuEnum.getMenuScopeByIdentifier(
            this.companyScopeMenu,
            orgManageButton.org_manage_add
          );
    ...
    methods: {
      ...
      clickExportOrg() {
        ...
        const tempMenu = scopeMenuEnum.getMenuScopeByIdentifier(
          this.companyScopeMenu,
          orgManageButton.org_manage_export
        );

        if (!tempMenu.isHasPermission) {
          this.$message.error('组织批量导出失败：您当前职位无权限操作');
        } else {
      ...
      getStoreList() {
        ...
        const params = {
          ...
          parentDeptIdList: this.deptIdList,
          isDept,
          ...this.filterForm,
        };
