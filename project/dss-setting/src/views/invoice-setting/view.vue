<template>
  <div class="invoice-wrapper">
    <div class="wkt-opt-label-container" style="padding: 26px 20px;">
      <div class="title">
        <span style="font-weight: 600;">发票功能</span>
        <div class="switch-wrapper"
             @click="handleClick">
          <el-switch v-model="useInvoice"
                     @change="hanleSwitch"
                     :disabled="isOpen"
                     :active-value="open"
    ...
    <div class="invoice-list">
      <el-row>
        <el-table :data="invoiceList"
          ...
          <el-table-column label="开票项编码"
                           min-width="100"
                           prop="spbm">
          </el-table-column>

          <el-table-column label="开票项名称"
                           min-width="100"
                           prop="spmc">
          </el-table-column>
          ...
          <el-table-column label="默认模板"
            ...
            <template slot-scope="scope">
              <el-switch v-model="scope.row.defaultFlag"
                         active-color="#4379ed"
    ...
    <msg-dialog :visible.sync="isShow"
                :title="content.title"
                :msg="content.msg"
                @handleConfirm="handleConfirm"></msg-dialog>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    handleConfirm() {
      this.content.flag === 'ENT'
        ? this.$router.push('/invoice-setting/edit-company')
    ...
    onAlter(row) {
      this.$router.push(`/invoice-setting/edit?id=${row.id}`);
    },
    handleDel(tpl) {
