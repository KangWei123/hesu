...
(function($){
  ...
  BarrageBuilder.prototype.initialize = function () {
    //this.options.css.left = this.$container.width() + randomNumber(0, 110);
    this.options.css.transform =
      'translate3d('+(this.$container.width() + randomNumber(0, 110))+'px,0,0)'
    ;
  ...
  BarrageBuilder.prototype.animation = function (point, time) {
    ...
    this.element.one('transitionend webkitTransitionEnd oTransitionEnd otransitionend MSTransitionEnd',
      onFinish);
    
    this.element.css({
      'opacity': 0.6,
      'transform':'translate3d('+point+'px,0,0)',
      'transition':'transform '+time+'ms linear,opacity '+time+'ms linear'
    });
