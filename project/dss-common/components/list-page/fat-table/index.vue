...
<script>
  import { hasSlots } from '@/dss-common/utils/vue';
    ...
    methods: {
      ...
      async confirmDelete(ids) {
        try {
          ...
          await this.$confirm(message, this.removeConfirmTitle, {
            type: 'warning',
            center: this.removeIsCenter,
            customClass: this.removeClass,
          });
          return true;
        } catch (err) {
          console.log(err);
