  ...
  export default {
    ...
    methods: {
      operating(item) {
        ...
        if (this.globalIsUmpPlatformApp || eventKey === 'event_consume' || eventKey === 'event_deduction') {
          return [
            {
              name: '详情',
              link: {
            ...
            {
              name: '编辑',
              link: { path: '/score-center/give-rule/form', query: { id: this.globalIsUmpPlatformApp ? id : refId } },
            },
            {
              name: '删除',
              onClick: () => {
                  ...
                  memberApi.scoreCenter
                    ...
                    .then(({ data }) => {
                      ...
                      if (data.length) {
                        ...
                        this.tipContent = `警告：【${eventName}】积分动作权限下目前有未结束的活动，删除后，适用应用下相关动作下的事件将直接失效，请确认是否删除？`;
                        this.tipContentSub = msg;
                      } else {
                        this.tipContent = `警告：删除【${eventName}】积分动作权限，将导致适用应用无法继续创建对应动作下的事件，请确认是否删除？`;
                        this.tipContentSub = '';
                      }
      ...
      onToAdd() {
        if (!this.all) {
          this.$router.push('/score-center/give-rule/form');
        }
      },
      handleRemove(ids) {
