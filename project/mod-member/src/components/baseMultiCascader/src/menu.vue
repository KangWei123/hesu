<script>
  ...
  export default {
    ...
    render(h) {
      const { activeValue, activeOptions, visible, expandTrigger, popperClass, hoverThreshold, multiple } = this;
            ...
            >
              {multiple ? (
                <el-checkbox
                  ...
                  disabled={
                    item.disabled || (!changeOnSelect && Array.isArray(item.children) && item.children.length > 0)
      ...
      return (
        <transition name="el-zoom-in-top" on-before-enter={this.handleMenuEnter} on-after-leave={this.handleMenuLeave}>
          <div v-show={visible} class={['multi-el-cascader-menus el-popper', popperClass]} ref="wrapper">
