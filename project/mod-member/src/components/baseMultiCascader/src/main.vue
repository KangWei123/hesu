<template>
  <span
    class="multi-el-cascader multi-cascader"
    :class="[
      {
    ...
    >
      <template slot="suffix">
        <i
          ...
          @click="clearValue"
        ></i>
        <i key="2" v-else class="multi-el-input__icon el-icon-arrow-down" :class="{ 'is-reverse': menuVisible }"></i>
      </template>
    </el-input>
    <span
      class="multi-el-cascader__label"
...
<script>
  import Vue from 'vue';
  import ElMultiCascaderMenu from './_menu';
  ...
  const copyArray = (arr, currentPath, valueKey, childrenKey) => {
    ...
    arr.forEach((item = {}) => {
      if (typeof item === 'object' && !Array.isArray(item)) {
        const itemCopy = {};
        const configurableProps = Object.keys(item);
      ...
      highlightKeyword(label, keyword) {
        ...
        return label
          ...
          .map((node, index) =>
            index === 0
              ? node
              : [h('span', { class: { 'multi-el-cascader-menu__item__keyword': true } }, [this._v(keyword)]), node]
          );
      },
        ...
        if (index > -1 && !this.cascaderDisabled) {
          ...
          if (this.selectChildren) {
            ...
            item.forEach((ele, idx) => {
              const currentPath = item.slice(0, idx + 1);
              const currentIndex = value.findIndex(ele => this.menu.isArrayEqual(currentPath, ele));
              if (currentIndex !== -1) {
            ...
            // 删除子节点
            value = value.filter(
              ele => !(ele.length > item.length && this.menu.isArrayEqual(item, ele.slice(0, item.length)))
            );
          } else {
            const currentIndex = value.findIndex(ele => this.menu.isArrayEqual(item, ele));
            if (currentIndex !== -1) {
              value.splice(currentIndex, 1);
            }
          }
...
<style lang="css" src="./cascader.css"></style>
<style lang="less">
  .multi-cascader {
    .multi-base-input {
      input {
