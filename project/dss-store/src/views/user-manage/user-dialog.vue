  ...
  <el-dialog title="用户详情"
    ...
    <div class="user-from">
      <div class="head">
        ...
        <div class="user-box">
          <div class="user-item"><span>用户ID：</span>{{userData.customerId}} <span style="margin-left:50px">姓名：</span>{{userData.name}}</div>
          <div class="user-item"><span>性别：</span>{{userData.sex}} <span style="margin-left:84px">年龄：</span>{{userData.age}}</div>
          <div class="user-item"><span>手机：</span>{{userData.phone||'无'}} <span style="margin-left:96px">到店次数：</span>{{totalCount||userData.enterCount}}</div>
          <!-- <div class="user-item"><span>是否会员：</span>{{userData.memberType === 1? '是': '否'}}
            <a class="dss-link"
               @click="gotoMember"
               href="javascript:;"
               v-if="userData.memberType === 1 && userData.memberId">查看更多</a></div> -->
          <div class="user-item"><span>用户类型：</span>{{curUserType}}
            <a class="dss-link"
               @click="gotoMember"
               href="javascript:;"
               v-if="userData.memberType === 1 && userData.memberId">查看更多</a></div>
      ...
      <div class="user-foot">
        <div class="user-item"><span>备注：{{userData.remark}}</span></div>
        ...
        <div class="store-table">
          ...
          <template>
            ...
            <div v-else
                 class="store-table-title"
                 style="padding: 4px 0;">暂无数据</div>
          </template>
...
// import memberApi from '@/dss-member/src/api/memberAPI.js';
