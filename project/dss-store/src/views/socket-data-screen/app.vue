      ...
      <url-loader-socket url="/user/topic/faceHistory/flow/"
        ...
        <div slot-scope="{ isLoading, response }">
          <grid-item :x="575"
                     ...
                     :h="58"
                     :w="257">
            <flow-panel :data="response.data ? response.data.ctDay : {}"
                        :borderLeft="true"></flow-panel>
          </grid-item>
          <grid-item :x="832"
                     :y="130"
                     :h="58"
          ...
          <grid-item :x="1089"
                     ...
                     :h="58"
                     :w="257">
            <flow-panel :data="response.data ? response.data.ctMonth : {}"
                        :borderRight="true"></flow-panel>
          </grid-item>
        </div>
      </url-loader-socket>
      ...
      <url-loader-socket url="/user/topic/faceHistory/list/anchor/"
        ...
        <div slot-scope="{ isLoading, response }">
          ...
          <grid-item :x="0"
                     ...
                     :h="861"
                     :w="185">
            <flow-list :data="response.data || []"
                       @choose='setIndex'></flow-list>
          </grid-item>
          <!-- 用户标签 -->
          <grid-item :x="320"
                     :y="251"
                     :h="357"
                     :w="445">
            <user-label :wkbStatus='wkbStatus'
          ...
          <grid-item :x="810"
            ...
            <user-info :wkbStatus='wkbStatus'
                       :data="response.data && response.data.length ? response.data[selectIndex] : {}"></user-info>
          </grid-item>
          <!-- 到店记录 -->
          <grid-item :x="1172"
                     :y="251"
                     :h="263"
                     :w="137">
            <arrive-record :wkbStatus='wkbStatus'
                           :data="response.data && response.data.length ? response.data[selectIndex] : {}"></arrive-record>
          </grid-item>
          <!-- 消费记录 -->
          <grid-item :x="1395"
                     :y="251"
                     :h="255"
                     :w="475">
            <expense-record :wkbStatus='wkbStatus'
                            :data="response.data && response.data.length ? response.data[selectIndex] : {}"></expense-record>
          </grid-item>
          <!-- 持有优惠券 -->
          <grid-item :x="1146"
                     :y="524"
                     :h="260"
                     :w="290">
            <coupon :wkbStatus='wkbStatus'
                    :data="response.data && response.data.length ? response.data[selectIndex] : {}"></coupon>
          </grid-item>
          <!-- 智能推荐 -->
          <grid-item :x="1455"
                     :y="806"
                     :h="204"
                     :w="416">
            <recommend :wkbStatus='wkbStatus'
                       :data="response.data && response.data.length ? response.data[selectIndex] : {}"></recommend>
          </grid-item>
          <!-- 用户价值 -->
          <grid-item :x="320"
                     :y="626"
                     :w="445"
                     :h="384">
            <radar :data="response.data && response.data.length ? response.data[selectIndex] : {}"
                   title="用户价值"
                   :max="5">
              <decoration slot="decoration"
                          :decoration="{left: [12], right: [29, 283]}" />
            </radar>
          </grid-item>
          <!-- 消费品类占比 -->
          <grid-item :x="1455"
          ...
          <grid-item :x="1146"
                     ...
                     :h="201"
                     :w="290">
            <browse-record :wkbStatus='wkbStatus'
                           :data="response.data && response.data.length ? response.data[selectIndex] : {}"></browse-record>
          </grid-item>
        </div>
      </url-loader-socket>
    </layout>
