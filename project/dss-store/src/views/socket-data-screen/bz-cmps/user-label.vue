<template>
  <div class="user-label blue-shadow-box">
    ...
    <div class="content">
      ...
      <div class="tags">
        <div class="tag"
             v-for="(tag,index) in jiGuangTag"
             :key="index">
          {{tag}}
        </div>
        <div class="no-data"
             v-if="jiGuangTag && !jiGuangTag.length">暂无标签</div>
      </div>
      ...
      <div class="tags">
        <div class="tag"
             v-for="(tag,index) in ImpressionList"
             :key="index">
          {{tag}}
        </div>
        <div class="no-data"
             v-if="ImpressionList && !ImpressionList.length">暂无标签</div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    tagJson: function(item) {
      let list = [];
      if (item.jiGuangTag) {
        let jiGuangTag = this.$plain(JSON.parse(item.jiGuangTag));
        //消费能力
