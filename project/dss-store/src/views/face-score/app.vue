      ...
      <url-loader url="/c/faceHistory/flow"
        ...
        <div slot-scope="{ isLoading, response }">
          <grid-item :x="49"
                     ...
                     :w="522"
                     :h="55">
            <sign-in :is-enter-face-score="isEnterFaceScore" :data="response.data ? response.data.ctMonth : {}" />
          </grid-item>
        </div>
      </url-loader>
      ...
      <url-loader-table :url="'/c/fuli/face/history_anchor' + (isEnterFaceScore && cosmetic ? ('?cosmetic=' + cosmetic) : '')"
        ...
        <div slot-scope="{ isLoading, response }">
          <grid-item :x="619"
                     ...
                     :h="941"
                     :w="682">
            <face-table :is-enter-face-score="isEnterFaceScore" :data="response.data || []"></face-table>
          </grid-item>
          ...
          <grid-item :x="1349"
                     ...
                     :w="522"
                     :h="450">
            <user-info :is-enter-face-score="isEnterFaceScore" :data="response.data && response.data.length ? response.data[0] : {}" />
          </grid-item>
        </div>
      </url-loader-table>
