<template>
  <WkbPageNav class="sorting-manage" page-name="经销商商户号" custom-content>
    <div class="sorting-area-list" v-loading="isLoading" :element-loading-text="msg.update">
      <div class="switch-container">
        ...
        <span class="title-tip"
          >开启后，经销商门店订单金额将入账经销商商户号，直营门店订单金额入账总部商户号；且支持经销商订单分账给品牌总部，设置品牌分账比例</span
        >
      </div>
      ...
      <set-dealer-dialog :dealer-obj="dealerObj" @done="getList(true)" v-model="setDealerDialogShow" />

      <set-sorting-dialog :store-id-list="settingList" @done="getList(true)" v-model="settingDialogShow" />
    </div>
  </WkbPageNav>
</template>
<script>
  import sortingApi from '@/mod-console/src/api/brand-setting/sorting';
  ...
  import { giftEnum, StoreSelectDialog } from '@/register/wechat3rd';
