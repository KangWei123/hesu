<template>
  <div class="function-buy-view" v-loading="loading">
    <div class="purchased" v-if="wkbStatus || klbStatus">
      <p class="title">已购功能</p>
      <div class="tab-box" v-for="(item, index) in functionList" :key="index">
        <div class="tab" v-if="activeTab('purchased', item.type)">
          ...
          <el-button type="primary" class="btn" @click="onOperation(item)">{{ btnText(item.btnType) }}</el-button>
          <p class="tips" v-if="item.tips">{{ item.tips }}</p>
        </div>
      </div>
    </div>
    <div class="more" v-if="!wkbStatus || !klbStatus">
      <p class="title">更多功能</p>
      <div class="tab-box" v-for="(item, index) in functionList" :key="index">
        <div class="tab" v-if="activeTab('more', item.type)">
          <div class="store-content">
            <div class="top-box">
              <img class="logo" :src="item.logo" />
              <p class="name">{{ item.name }}</p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      this.$router.push({
        path: '/brand-setting/function-buy/order?id=' + id
      });
    }, */
