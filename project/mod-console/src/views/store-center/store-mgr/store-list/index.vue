    ...
    <div class="store-container" v-loading="treeloading">
      ...
      <div class="table-box">
        ...
        >
          ...
          <el-table-column label="开通时间" prop="createTime" width="200"></el-table-column>
...
<script>
  ...
  import { exportStoreListNew } from '@/mod-console/src/api/storeApi';
  import * as orApi from '@/mod-console/src/api/organizationFramework';
  import DeptSelect from '@/mod-console/src/components/departmentSelect';
  import setStoreSuperior from '@/mod-console/src/components/setStoreSuperior';
  ...
  export default {
    ...
    methods: {
      ...
      hasStore(arr) {
        ...
        const dfs = _arr => {
          for (let i = 0; i < _arr.length; i++) {
            ...
            if (Array.isArray(_arr[i].children)) {
              dfs(_arr[i].children);
            }
          }
        };
      ...
      findParents(arr, target) {
        ...
        const dfs = (_arr, _target, path) => {
          for (let i = 0; i < _arr.length; i++) {
            if (res) break;
            if (_arr[i].deptId === _target) {
              res = _arr[i];
              parent = path;
              return;
            }
            if (Array.isArray(_arr[i].children)) {
              dfs(_arr[i].children, target, path.concat(_arr[i]));
            }
          }
        };
        dfs(arr, target, []);
      ...
      async submitRemoveTree() {
        ...
        try {
          await orApi.delOrganization(params).then(res => {
            this.getOrganizationTreeData();
            this.$message({
              type: 'success',
              message: '删除成功',
      ...
      handleCascaderChange(value) {
        ...
        const { parent } = this.findParents(this.organizationTree, this.currentOrganization.deptId);
        if (this.isAdd) {
      ...
      getOrganizationCascader(arr, ind, targetId) {
        const dfs = (_arr, num, _targetId) => {
          for (let i = 0; i < _arr.length; i++) {
            _arr[i].disabled = false;
            _arr[i].dep = num;
            if (_arr[i].type == 2 || _targetId == _arr[i].deptId || num == 4) {
              _arr[i].disabled = true;
            }
            if (Array.isArray(_arr[i].children)) {
              dfs(_arr[i].children, num + 1, _targetId);
            }
          }
        };
