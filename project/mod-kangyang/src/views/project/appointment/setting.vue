  ...
  <form-page-layout :title-visible="false">
    <div class="settings">
      ...
      >
        ...
        <el-form-item label="预约范围" prop="scope" class="form-item">
          可预约
          <el-input-number
            v-model="form.scope"
            style="width: 100px"
        ...
        <el-form-item label="预约时间" prop="weekDays">
          <el-select v-model="form.weekDays" multiple style="min-width: 500px">
            <el-option v-for="item of WEEKS" :key="item" :label="`星期${item}`" :value="`星期${item}`"></el-option>
          </el-select>
          <duration-select ref="durationSelect" v-model="form.timeRule" type="project"></duration-select>
        </el-form-item>

        <el-form-item label="不可预约日期" class="form-item">
          <el-date-picker
            v-model="form.unAppointDateList"
            type="dates"
            placeholder="请选择"
            format="yyyy-MM-dd"
            value-format="yyyy-MM-dd"
            :disabled="(form.unAppointDateList || []).length >= 10"
            :picker-options="pickerOptions"
          ></el-date-picker>

          <span class="tip" style="margin-left: 10px">例如节假日、公休日</span>

          <div class="tags">
            <el-tag
              v-for="(item, idx) of form.unAppointDateList"
              :key="item"
              type="info"
              closable
              @close="form.unAppointDateList.splice(idx, 1)"
              >{{ item }}</el-tag
            >
            <span>最多可添加 10 个</span>
          </div>
        </el-form-item>

        <el-form-item label="取消规则" prop="scope" class="form-item">
          预约者可提前
          <el-input-number
...
<script>
  ...
  import { projectAppointmentApi } from '@/mod-kangyang/src/api';
  ...
  export default {
    ...
    data() {
      return {
        ...
        pickerOptions: {
          disabledDate: () => {
            return (this.form.unAppointDateList || []).length >= 10;
          },
        },
        allProjects: [],
      };
    ...
    methods: {
      async handleGetDetail() {
        const { data } = await projectAppointmentApi.getSetting({
          id: this.settingId,
        });

        const { epId, appId, createBy, createTime, updateBy, updateTime, appointDateList, ...rest } = data;

        rest.timeRule.sort((a, b) => {
          return getTimeStamp(a.startTime) > getTimeStamp(b.startTime) ? 1 : -1;
        });
        rest.weekDays = (appointDateList || []).map(index => {
          return `星期${WEEKS[index - 1]}`;
        });
        rest.unAppointDateList = data.unAppointDateList || [];
      ...
      async handleSave() {
        ...
        payload.appointDateList = weekDays.map(item => {
        ...
        try {
          if (this.isEdit) {
            ...
            await projectAppointmentApi.updateSetting(payload);
          } else {
            await projectAppointmentApi.addSetting(payload);
          }
      ...
      async getProjects() {
        const { data } = await projectAppointmentApi.getAvailableProjects();
