...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.newparenStackedArea = this.parenStackedArea;
      if (this.newparenStackedArea && this.newparenStackedArea.length && this.fixXAxisData) {
        this.drawLine1();
      }
    },
    beforeDestroy() {
    ...
    methods: {
      drawLine1() {
        ...
        let tempperSecs = [];
        let tempbounceRate = [];
        this.margin.forEach(function(value) {
          tempdowncontent.push(value.statDate);
          tempvisitNum.push(value.visitNum);
          tempvisitor.push(value.visitor);
          tempnewUser.push(value.newUser);
          tempperSecs.push(value.perSecs < 0 ? 0 : value.perSecs);
          tempbounceRate.push(value.bounceRate);
        });
        that.downcontent = tempdowncontent;
        that.visitNum = tempvisitNum;
        that.visitor = tempvisitor;
        that.newUser = tempnewUser;
        that.perSecs = tempperSecs;

        tempbounceRate.forEach(item => {
          that.bounceRate.push(parseFloat(item * 100).toFixed(2));
        });
        // that.bounceRate = that.bounceRate;
